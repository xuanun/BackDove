...
class PlatformController extends Controller
{
    ...
    public function  addFirmUser(Request $request)
    {
        ...
        if(empty($rsg_time) || empty($firm_name) || empty($firm_icon) || empty($role_name) || empty($role_desc) || empty($user_name) || empty($mobile))
        ...
        //新增用户角色信息
        $role_data = $model_roles->addUserRole($user_data['user_id'], $role_data['role_id']);
        if($role_data['code'] != 20000)
        {
            DB::rollBack();
            return response()->json($role_data);
    ...
    public function  warehouseList(Request $request)
    {
        ...
        $model_Cage = new Cage();
        $block_data = $model_block->getBlockList($factory_id, $block_type, $block_id, $firm_id);
        $return_data = array();
        foreach ($block_data as $v)
        {
            $cage_ids = $model_Cage->getCage($v->id, $start_cage, $end_cage);
            $ids_array = array();
            foreach ($cage_ids as $value)
            {
                $ids_array[] = $value->id;
            }
            $chunk_ids = array_chunk($ids_array, $size);
            for($i = 0; $i < count($chunk_ids); $i++)
            {
                $arr = array();
                $arr['id'] = $v->id;
                $arr['factory_name'] = $v->factory_name;
                $arr['name'] = $v->name;
                $arr['block_type'] = $v->block_type;
                $arr['type_name'] = $v->type_name;
    ...
    public function  warehouseAdd(Request $request)
    {
        ...
        {
            $cage_data = $model_Cage->addCage($block_data['block_id']);
...
//        $cage_amount =  isset($input['cage_amount']) ? $input['cage_amount'] : '';
        ...
        foreach ($params as $v)
        {
            ...
            for($i=0; $i<$v['cage_amount']; $i++)
            {
                $cage_data = $model_Cage->addCage($v['block_id']);
                if($cage_data['code'] != 20000){
                    DB::rollBack();
                    return response()->json($cage_data);
                }
